...
QString HexParser::saveHexFile(const QString &fileName)
{
    ...
    foreach (auto row, m_hexData) {
        dataFile.write(row.toUpper() + "\r\n");
    }
...
void HexParser::rewriteValue(int value_index, const QByteArray &new_value)
{
    ...
    int index_to_rewrite = m_valueSectionIndex + (value_index%4 == 0 ? value_index/4 - 1 : value_index/4) + 1;
...
void HexParser::setLorawanKeys(const QByteArray &appEUI, const QByteArray &devEUI, const QByteArray &appKey)
{
    m_appEUI = appEUI;
    m_devEUI = devEUI;
    m_appKey = appKey;
